...
export const loader: LoaderFunction = async ({ request, params }) => {
  const tipoEntidad = "MARPRO";
  const { idMarcaProducto } = params as { idMarcaProducto: string };
  const data = await getImagenesTipoEntidad({ request, tipoEntidad, idEntidad : idMarcaProducto });
  return data;
}

export default function CMSDefinirMarcasProductosList() {
...
//REMIX
  const handleEditarImagen = () => {
    navigate(`/lista/CMSDefinirMarcasProductos/${idMarcaProducto}/${nombreMarca}/edit`)
  }
//REMIX
  const handleCambiarMarca = () => {
    navigate("/CMSDefinirMarcasProductos")
  }
    ...
    <>
      ...
      <ExcelExport data={result.data} ref={_export}>
        ...
        >
          ...
          <GridColumn columnMenu={ColumnMenu} field="mimeType" title="MineType" />
