...
export const action: ActionFunction = async ({ request, params }) => {
  const { idMarcaProducto = "", nombreMarca } = params;
  ...
  const response = await postSetImagen({ request, tipoEntidad, idEntidad: idMarcaProducto, data });

  if (!response.ok) {
    return json({ statusText: response.statusText, status: response.status, data });
  }

  return redirect(`/lista/CMSDefinirMarcasProductos/${idMarcaProducto}/${nombreMarca}`);
}
