...
//ACTION FUNCTION
 export const action: ActionFunction = async ({ request, params }) => {
  const {  idEntidad, tipoEntidad } = params as { idEntidad: string, tipoEntidad: string };
  ...
  const response = await postSetImagen({ request, tipoEntidad,  idEntidad, data });

  if (!response.ok) {
    return json({ statusText: response.statusText, status: response.status, data });
  }

  return redirect(`/CMSOtrosContenidos`);
} 

export default function CMSOtrosContenidosEdit() {
