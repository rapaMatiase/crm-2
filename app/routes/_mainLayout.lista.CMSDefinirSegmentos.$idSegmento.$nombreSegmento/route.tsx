...
export const loader: LoaderFunction = async ({ request, params }) => {
  const tipoEntidad = "SEG";
  const { idSegmento } = params as { idSegmento: string };
  const data = await getImagenesTipoEntidad({ request, tipoEntidad, idEntidad : idSegmento });
  
  return data;
}

export default function CMSDefinirSegmentosList() {

  const data = useLoaderData()
  const navigate = useNavigate();
  const {  idSegmento, nombreSegmento } = useParams();
...
//REMIX
  const handleEditarImagen = () => {
    navigate(`/lista/CMSDefinirSegmentos/${idSegmento}/${nombreSegmento}/edit`);
  }
//REMIX
  const handleCambiarSegmento = () => {
    navigate("/CMSDefinirSegmentos")
  }
  ...
  return (
    <>
      <h2> {nombreSegmento} </h2>
      <Button onClick={handleCambiarSegmento}> Cambiar Segmento </Button>
