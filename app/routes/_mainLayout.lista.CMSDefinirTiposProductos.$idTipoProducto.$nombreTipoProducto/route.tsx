...
export const loader: LoaderFunction = async ({ request, params }) => {
  ...
  const { idTipoProducto } = params as { idTipoProducto: string };
  const data = await getImagenesTipoEntidad({ request, tipoEntidad, idEntidad: idTipoProducto });
  return data;
}

export default function CMSDefinirTiposProductosList() {

  const data = useLoaderData()
  const navigate = useNavigate();
  const { idTipoProducto, nombreTipoProducto } = useParams();
  ...
  const handleEditarImagen = () => {
    navigate(`/lista/CMSDefinirTiposProductos/${idTipoProducto}/${nombreTipoProducto}/edit`);
  }
  //REMIX
  const handleCambiarTipoProducto = () => {
    navigate("/CMSDefinirTiposProductos")
  }
  ...
  return (
    <>
      <h2> {nombreTipoProducto} </h2>
      <Button onClick={handleCambiarTipoProducto}> Cambiar Tipo de Producto </Button>
