...
function createComponent(config: any, data: Record<string, any>, setProducto: any): React.ReactNode {
  ...
  const parsedContent = Array.isArray(content)
    ? content.map((child, index) => createComponent(child, data, setProducto))
  ...
  Object.keys(events).forEach(event => {
    if (event.startsWith("on")) {
      if (events[event] === "#TODO#") {
        props[event] = (event: any) => AddToCardTodo(event, EsVendible, EsOferta, EsAlquilable);
      } else if (events[event] === "#ADD#") {
        props[event] = (event: any) => AddToCard(event);
      } else if (events[event] === "#OFERTA#") {
        props[event] = (event: any) => AddToCardOferta(event);
      } else if (events[event] === "#ALQUILAR#") {
        props[event] = (event: any) => AddToCardAlquilado(event);
      } else {
